...
library(scales)
# data input
pre_data <- read.table("/Users/dangliu/Desktop/JT_lab/Synteny/CEvsCBG/test2.input.txt", header = F)
data2 <- read.table("/Users/dangliu/Desktop/JT_lab/Synteny/CEvsCBG/test3.input.txt", header = F)
...
#p <- p + annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size=1)
# title setting
#p <- p + ggtitle(expression(paste(italic("C. elegans")," vs. fragmented ",italic("C. briggsae"))))
