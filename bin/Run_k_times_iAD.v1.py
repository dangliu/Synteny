...
run = 1
out_f = open(dir_path + '/SynCov/SynCov_iAD.' + str(n) + 'kb.txt', 'a')
...
for i in range(int(k)):
	...
	subprocess.call('python ../../../bin/blast_file_process_foriAD.py ' + ref + 'vs' + qry + '.txt', shell=True)
	subprocess.call('cp out_' + ref + 'vs' + qry + '.txt ./' + str(n) + 'kb.blast.txt', shell=True)
	subprocess.call('cp -s ../../../data_source/gffs/' + ref + '.coding_gene.gff ./', shell=True)
	subprocess.call('cp -s ../../../frag_assemblies/' + ref + 'vs' + qry + '/For_iAD/' + qry + '.' + str(n) + 'kb.coding_gene.gff ./', shell=True)
	...
	# run i-adhore
	subprocess.call('../../bin/i-adhore-3.0.01/bin/i-adhore ' + str(n) + 'kb.ini >log 2> err', shell=True)
	# to sub directory
	os.chdir(dir_path + '/raw_process/iAD/' + ref + 'vs' + qry + '/' + str(n) + 'kb_output')
	subprocess.call('python ../../../../bin/iAD_out_transform.py ../../../../frag_assemblies/' + ref + 'vs' + qry + '/For_iAD/' + qry + '.' + str(n) + 'kb.coding_gene.gff ../../../../data_source/gffs/' + ref + '.coding_gene.gff segments.txt', shell=True)
	subprocess.call('python ../../../../bin/python/synteny_gff.py transformed.segments.txt A', shell=True)
	subprocess.call('cat A.transformed.segments.txt.int* > pre.' + str(n) + 'kb.synteny.gff', shell=True)
	subprocess.call('python /mnt/nas1/dl/bin/python/gff_sorting.py pre.' + str(n) + 'kb.synteny.gff > qry.' + str(n) + 'kb.synteny.gff', shell=True)
	...
	subprocess.call('python ../../../../bin/synteny_gff.py transformed.segments.txt B', shell=True)
