...
library(scales)
# data input
pre_data <- read.table("/Users/dangliu/Desktop/JT_lab/Synteny/SRvsSR/test2.input.txt", header = F)
...
#get delta
data[data$Program=='DAG',]$Cov <- 0.9863 - data[data$Program=='DAG',]$Cov
data[data$Program=='iAD',]$Cov <- 0.9855 - data[data$Program=='iAD',]$Cov
data[data$Program=='MCS',]$Cov <- 0.9853 - data[data$Program=='MCS',]$Cov
data[data$Program=='SyC',]$Cov <- 0.9892 - data[data$Program=='SyC',]$Cov
...
#p <- p + annotate("segment", x=-Inf, xend=-Inf, y=-Inf, yend=Inf, size=1)
# title setting
#p <- p + ggtitle(expression(paste(italic("S. ratti")," vs. fragmented ",italic("S. ratti"))))
